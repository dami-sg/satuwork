...
**模型 id 怎么到席位手里。** `/runtime/catalog` 的响应里加一段：
...
- **Firecrawl** — 搜索（`/v2/search`）和提取（`/v2/scrape`）都有。提取那头是**真渲染的浏览器**，Tavily 拿不到正文的 SPA 页面用它。只用 `markdown` 这一个 format：它的 `summary` 和带 prompt 的 `json` 都是它自己的 LLM，而摘要在我们这儿一律走 utility（理由见第 9 节）。自托管的实例用 `FIRECRAWL_API_URL` 换地址，变量名和上游文档一致。
