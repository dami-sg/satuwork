...
| 开关 | 判据 | 拦不住什么 |
|---|---|---|
| 禁止访问未授权的外部系统 | MCP 工具必须属于这颗 Bot 挂上的服务器（或这一轮 `@` 点名的连接）；`bash` 扫命令名（`curl` / `git push` / `pip install` …），`bash -c "…"`、`eval "…"` 里那一段先展开再扫 | `bash` 里 base64 拼出来的命令。真正的解是把 bash 关进沙箱，在那之前这一层是「挡住顺手」，不是「挡住蓄意」 |
...
**流上为什么只垫 1 轮**：侧栏名单给每个 Bot 都挂一条流，用来显示「在不在跑 / 最近说了
什么 / 什么时候说的」。这三样一轮就够，而二十轮乘几个 Bot 全要在浏览器主线程上
`JSON.parse`。打开对话要看的那二十轮，改成人真的点进去时才拉。
