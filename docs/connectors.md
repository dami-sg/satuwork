...
3. **退化成元工具**：超限时该连接只出 `search(query)` / `describe(tool)` /
   `execute(tool, args)` 三个。`caps.search` 为假的供应商没有第 3 条，只能靠前两条
...
现在的余额（`gateway/src/lib/billing.ts` 的 `balanceOf`）是「套餐赠送 + 充值」，注释里写着
「消耗还没接」。连接器是**第一个真的往下扣的东西**，所以口径在这里定死：
...
**两个桶必须分开记，流水行上要留下这一笔吃了哪个桶**（`connector_calls.bonusMicros`）。
