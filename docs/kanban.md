...
> **这一版的执行面已经整个删掉，看现在的那份：[task-board.md](./task-board.md)。**
>
> 看板不再派活：卡、板、成员名单、调度器、卡片会话、`kanban_*` 那七把工具、席位上的
> `/api/cards` 全部不存在了（删除清单见 task-board.md §14）。今天的任务是**从对话里被
> 认出来的**，只给人看，不驱动任何东西。
>
> 这一份留着不是为了对照实现，是为了那几段推理：为什么去重指纹必须带时间桶、为什么
> 「派卡的那一刻没有人在等着看」决定了几乎所有取舍、为什么一开始不做拖拽——下一个动这块
> 地方的人还用得上。**读它时记住：下面每一句关于「怎么跑起来」的话都已经不成立。**
...
### 10.6 执行方那些调用真要人拍板时，卡片开在哪儿

**开在那颗 Bot 自己的长会话上，不是卡片会话上。**

上面那句「真正会出去的动作在那颗 Bot 手上，到时候单独过闸」是有前提的：过闸过得下去。
卡片会话 `kind: 'card'`，[session/index.ts:159](../bot/src/session/index.ts:159) 那条过滤把它
挡在侧栏之外——审批卡开在它身上，人在界面上根本找不到那张卡，五分钟后按超时收口，而他从头
到尾不知道有人问过他。一张要发邮件的卡会在**每一次** `external + write` 上白等满五分钟，
60 步的预算几次就撞穿 Gateway 那道墙钟（§12），整张卡按失败重派、重派之后再撞一遍。

做法抄 [delegation.md §6.2](./delegation.md)（子代理的卡片开在主会话上），只是卡没有「主会话」
这个现成的东西——它不是某次工具调用开出来的，`rootOf` 查不到它。所以 `runCard` 开跑前先把这
颗 Bot 那条长会话记进 `cards`（`homeSessionId`），`ApprovalGate.ask` 按
`rootOf → cardHomeOf → 自己` 这个顺序回落。

卡片上要写清出处（`来自板上的卡《…》`）：那张卡是后台派下来的，人可能连「现在有活在跑」
都不知道，而确认框却开在他自己那条对话里。

**不走 `parents`。** 那张表还兼着两件别的事——`mentionedIn` 顺着它继承这一轮的 `@` 点名、
`tools/index.ts` 顺着它给 `rebind` 的工具改道——而卡跑在自己的一条线上，既不该继承人这一轮
点的名，也不该让工具去摸人的那条会话。

没人在场时它照旧超时收口，然后走 `ESCALATE_AFTER` 那一档：连着被挡三次，话术说的是**调
`kanban_block`**，不是 `escalate_to_human`（那把工具在这儿不存在，见 §10.4）。
