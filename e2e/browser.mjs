...
export async function runBrowser({ root, test, assert, log }) {
  ...
  await test('链接要把地址带出来：正文里带、结构化那份也带', () => {
    /**
     * 早先快照只写「- link "名字" [@e12]」——名字有、地址没有。后果是模型手上从来就没有
     * 链接可给：让它列十个搜索结果，它只能给出十个标题，人拿到之后还得自己再搜一遍；
     * 界面上也无从展示，因为压根没有 URL 这个东西。
     *
     * 两份都要：正文那份给模型（它要引用第四十条时在正文里找得到），结构化那份给界面
     * （不然「展示链接」就得指望模型愿意把地址抄进回答里，而它天然倾向于只写标题）。
     *
     * 顺带钉两条边界：**超长的地址整条丢掉，不截断**——截出来的不是更短的地址，是错的
     * 地址，却照样以可点链接的样子摆到人面前；以及 `javascript:` 这类假协议在这一层就
     * 滤掉（界面那层还会再滤一次，两道都要）。
     */
