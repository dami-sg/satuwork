...
export class Db {
  ...
  // ── 任务看板：从对话里总结出来的任务（见 docs/task-board.md）─────────────
  //
  // 归属一句话：**一条任务属于一个员工的一颗 Bot 的一条会话**。所以这一整段里没有一个
  // 方法收 `companyId` 当判据——判据一律是 `tasks.accountId`，别人一律当成没有（404）。
  //
  // **这里没有任何一个方法能让一条任务跑起来。** 原来那一段（选卡、抢卡、收心跳、
  // 收死的）整个删了，不是移到了别处。

  /** 一条任务。别人的、不存在的，判据在 lib/tasks.ts，这里只管取。 */
  async task(id: string): Promise<Task | undefined> {
    const r = await this.one('select * from tasks where id = ?', [id])
    return r ? taskOf(r) : undefined
  }

  /**
   * 板上那一屏。keyset 翻页，排序键 (`stateAt` desc, id desc)。
   *
   * **不用 OFFSET**（同会话索引那条）：抽取器随时可能把某条任务的状态推一格，那一行就
   * 跳到列首，OFFSET 翻页要么漏要么重。
   */
  async tasksOf(
    accountId: string,
    filter: { botId?: string; state?: TaskState; limit?: number; before?: { stateAt: number; id: string } } = {},
  ): Promise<Task[]> {
    let sql = 'select * from tasks where "accountId" = ?'
    ...
    if (filter.before) {
      sql += ' and ("stateAt" < ? or ("stateAt" = ? and id < ?))'
      args.push(filter.before.stateAt, filter.before.stateAt, filter.before.id)
    }
    // 多要一条，路由拿它判 hasMore（同 sessionsPage）。
    const limit = Math.min(Math.max(Math.trunc(filter.limit ?? TASK_PAGE_DEFAULT), 1), TASK_PAGE_MAX + 1)
    sql += ' order by "stateAt" desc, id desc limit ?'
    args.push(limit)
    const rows = await this.many(sql, args)
    return rows.map(taskOf)
  }

  /** 四列各有几条。界面上的列头，和「我这儿还有几件事悬着」那个数。 */
  async taskCounts(accountId: string, botId?: string): Promise<Record<TaskState, number>> {
    const rows = await this.many(
      `select state, count(*)::int as n from tasks where "accountId" = ?${botId ? ' and "botId" = ?' : ''} group by state`,
      botId ? [accountId, botId] : [accountId],
    )
    const out: Record<TaskState, number> = { proposed: 0, doing: 0, done: 0, dropped: 0 }
    for (const r of rows) out[parseTaskState(r.state)] = num(r.n)
    return out
  }
   ...
   * 这条会话现在挂着哪些任务。
   *
   * 两个用处，而且是同一份事实的两面：抽取时**回喂给模型**那份清单（让它认出「这一段是
   * 在推进其中某条」而不是又开一条，见 docs/task-board.md §4.3），和那条每会话上限的
   * 计数。
   */
  async tasksOfSession(sessionId: string): Promise<Task[]> {
    const rows = await this.many('select * from tasks where "sessionId" = ? order by "firstSeq"', [sessionId])
    return rows.map(taskOf)
  }

  async taskByKey(sessionId: string, key: string): Promise<Task | undefined> {
    const r = await this.one('select * from tasks where "sessionId" = ? and key = ?', [sessionId, key])
    return r ? taskOf(r) : undefined
  }
   ...
   * 新认出来的一件事。
   *
   * **撞唯一键回 undefined，不抛**：`unique (sessionId, key)` 是并发下最后那道闸——同一
   * 条会话的两次抽取重叠时（席位重启后水位倒回去，或者去抖没拦住），两边给的是同一个
   * key。调用方接住 undefined 之后去 upsert 那一条，而不是把一次正常的竞态报成故障。
   */
  async insertTask(input: {
    ...
    const row: Task = {
      ...
      extractVersion: input.extractVersion ?? 0,
      createdAt: now,
      updatedAt: now,
      stateAt: now,
      doneAt: input.state === 'done' ? now : null,
    }
    try {
      await this.run(
        'insert into tasks (id, "accountId", "companyId", "botId", "sessionId", title, summary, state, key, "firstSeq", "lastSeq", evidence, "humanFields", "extractModel", "extractVersion", "createdAt", "updatedAt", "stateAt", "doneAt") values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
        [row.id, row.accountId, row.companyId, row.botId, row.sessionId, row.title, row.summary, row.state, row.key, row.firstSeq, row.lastSeq, row.evidence, JSON.stringify(row.humanFields), row.extractModel, row.extractVersion, row.createdAt, row.updatedAt, row.stateAt, row.doneAt],
      )
    } catch (e) {
   ...
   * 改一条任务。
   *
   * **`stateAt` / `doneAt` 由调用方显式传**，这里不替它算：状态没变的那种更新（抽取器
   * 只是把 `lastSeq` 往后推）绝不能动 `stateAt`，否则界面上「停滞 N 天」永远不会出现
   * （见 Task.stateAt 上那段）。把这个判断藏进 update 里，两个调用方总有一个会写错。
   */
  async updateTask(
    id: string,
    patch: {
    ...
    if (patch.firstSeq !== undefined) put('firstSeq', patch.firstSeq)
    if (patch.lastSeq !== undefined) put('lastSeq', patch.lastSeq)
    if (patch.evidence !== undefined) put('evidence', patch.evidence)
    if (patch.humanFields !== undefined) put('humanFields', JSON.stringify(patch.humanFields))
    if (patch.extractModel !== undefined) put('extractModel', patch.extractModel)
    if (patch.extractVersion !== undefined) put('extractVersion', patch.extractVersion)
    if (patch.stateAt !== undefined) put('stateAt', patch.stateAt)
    if (patch.doneAt !== undefined) put('doneAt', patch.doneAt)
    put('updatedAt', Date.now())
    args.push(id)
    await this.run(`update tasks set ${sets.join(', ')} where id = ?`, args)
    return this.task(id)
  }

  /** 认错的那一条，人删得掉。时间线跟着走（外键 on delete cascade）。 */
  async deleteTask(id: string): Promise<boolean> {
    return (await this.run('delete from tasks where id = ?', [id])) > 0
  }

  async taskEvents(taskId: string, limit = 50): Promise<TaskEvent[]> {
    const rows = await this.many('select * from task_events where "taskId" = ? order by "createdAt" limit ?', [
      taskId,
      Math.max(1, Math.trunc(limit)),
    ])
    return rows.map(taskEventOf)
  }
