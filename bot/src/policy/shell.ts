 ...
 *
 * `bash` 是这台席位上唯一一把什么都做得到的工具：它能写文件、能删目录、也能
 * `curl -d @secret https://…`。「禁止访问未授权的外部系统」这条开关如果对 bash 视而
 * 不见，那它挡住的只是老老实实走 MCP 的那条路——而真要外传数据的人根本不会走那条。
 *
 * 两条都要说清楚：
 *
 *  1. **不注册 bash 才是严密的做法**，但代价是这条边界（出厂默认就开着）一打开，
 *     全公司的 Bot 就都不会用命令行了。那不是收紧边界，那是换掉产品。
 *  2. 这里做的是**命令名匹配**：直白写出来的 `curl` / `ssh` / `git push` 拦得住，
 *     `bash -c "…"` / `eval "…"` 里那一段会被展开后照样匹配（见 NESTED_PAYLOAD），
 *     但 `echo Y3VybA== | base64 -d | sh` 拦不住。真正的解是把 bash 关进沙箱
 *     （见 tools/workspace.ts 顶上的说明），在那之前这一层是「挡住顺手」，不是
